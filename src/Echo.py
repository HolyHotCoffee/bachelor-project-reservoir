...
from reservoirpy.nodes import Reservoir, Ridge, FORCE, ESN, FORCE
...
def myESN(ticker, START, END, n_prediction,
          seed, lr, sr, warmup, neurons, rc_connectivity, i_c):
    
    # Creating our reservoir
    reservoir = Reservoir(neurons, lr=lr, sr=sr, seed=seed,
    ...
    my_pred = esn_model.run(stock_data[n_training:],
                           forced_feedbacks=Y_train,
                           shift_fb=True,
                           reset=False)
    ...
    table = plotting.datatable(START, END, n_prediction, seed, lr, sr, warmup, neurons, rc_connectivity, i_c)
