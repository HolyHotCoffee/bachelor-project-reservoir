################################################
##### Analyzing tools for our predictions ######
################################################
import math
import numpy as np
import pandas as pd
...
def get_ticker_opt_dirs():
    ticker_dirs = [ name for name in os.listdir('optimize-stocks/') 
...
def BEST_OPT_ESN(path, n_prediction, neurons, ticker, START, END):
    ...
    Echo.myESN(ticker, START, END, n_prediction,
            seed, lr, sr, warmup, neurons, rc_connectivity, input_connectivity)
...
def Mc_Simulation(df, n_simulations, boolPlot):
    ...
    t_1 = t_1.to_numpy()
    log_returns  = t_0 - t_1
    mu = np.mean(log_returns) # mu - sigma^2 -> Aendre parameter vejen.  -> Her divdere med dt
    # v = np.mean(log_return) / dt
    sigma = np.std(log_returns)  # Divder med kvadratrod dt, for at faa den rigtige sigma
